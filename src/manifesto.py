...
class Manifesto(object):
    ...
    def __init__(self, key):
        ...
        self.manifesto_functions = {
            "versions": "api_list_core_versions.json",
            "main": "api_get_core.json",
            "meta": "api_metadata.json",
            "text": "api_texts_and_annotations.json",
            "metaversions": "api_list_metadata_versions.json",
            "corecitation": "api_get_core_citation.json",
            "corpuscitation": "api_get_corpus_citation.json",
        }
    ...
    def __mp_request__(self, fun, params=None, version=None):
        ...
        if (
            fun == self.manifesto_functions["text"]
            or fun == self.manifesto_functions["meta"]
        ):
            ...
            if fun == self.manifesto_functions["text"]:
                ...
                return requests.get(
                    ...
                    + self.key_text
                    + "&"
                    + self.__meta_format__(params)
                    + version
                )

        elif not params == None:
            return requests.get(
                self.manifesto_api + fun + self.key_text + "&key=" + params
            )
    ...
    def mp_maindataset(self, version="current"):
        ...
        self.mp_dataset["edate"] = pd.to_datetime(
            self.mp_dataset["edate"], dayfirst=True
        )
    ...
    def mp_meta(self, version="current", keys=None, date=None, country=None):
        ...
        if country is not None and date is not None:
            if len(date.split()) > 1:
                key_inds = self.mp_dataset[
                    (
                        self.operators[date.split()[0]](
                            self.mp_dataset["edate"], date.split()[1]
                        )
                    )
                    & (self.mp_dataset["countryname"] == country)
                ]
                keys = list(key_inds["party"] + "_" + key_inds["date"])

            else:
                key_inds = self.mp_dataset[
                    (self.mp_dataset["edate"] == date)
                    & (self.mp_dataset["countryname"] == country)
                ]
                keys = list(key_inds["party"] + "_" + key_inds["date"])
